---
name: scaffold-dotnet-webapi
description: Scaffolds a complete .NET 10 Web API solution from scratch, asking the user for project name, database provider, architecture pattern (Clean/Onion/Repository+UoW/Vertical Slice), authentication, multi-tenancy, Docker, testing, cloud provider, and third-party HTTP integrations before generating full, compilable code. Use when the user asks to scaffold, generate, or bootstrap a new .NET Web API project or solution.
---
...
> |-----------|-----------------|
> | 1 — Clean Architecture | `references/clean-architecture.md` |
> | 2 — Repository + Unit of Work | `references/repository-uow-architecture.md` |
> | 3 — Onion Architecture | `references/onion-architecture.md` |
> | 4 — Vertical Slice Architecture | `references/vertical-slice-architecture.md` |
>
...
## Step 7 — Claude Memory File (Always Generate)

Always generate a detailed `CLAUDE.md` file at the root of the solution. This file must contain everything about the project so Claude can assist during future development sessions.

The `CLAUDE.md` file **must** include all of the following sections, populated based on the user's answers:
...
`.claude/agents` contains some reusable agents which can be directly copied over to the correct directory, so that they can be further used in the newly generated solutions
...
- [ ] `CLAUDE.md` — fully populated based on all user selections (architecture, auth, cloud provider, HTTP clients, multi-tenant, testing, Docker)
- [ ] Copy agents from `.claude/agents` to correct directory
